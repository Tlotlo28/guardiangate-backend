...
from app.schemas.admin import (
    LearnerCreate, AdminLearnerOut,
    AdminUserCreate, AdminUserOut,
...
def list_users(
    ...
    stmt = select(User).where(User.school_id == current_user.school_id)
    if role is not None:
...
def learner_guardians(
    ...
    if learner is None or learner.school_id != current_user.school_id:
        raise HTTPException(status_code=404, detail="Learner not found")
    links = db.execute(
        select(GuardianLink).where(GuardianLink.learner_id == learner_id)
    ).scalars().all()
    return [
        GuardianLinkOut(
            id=l.id,
            ...
            guardian_name=l.guardian.full_name,
            relationship_label=l.relationship_label,
            can_collect=l.can_collect,
        )
        for l in links
    ]
...
def badges_zip(
    ...
    with zipfile.ZipFile(buf, "w") as z:
        for l in learners:
            z.writestr(
                l.full_name.replace(" ", "_") + "_badge.png",
